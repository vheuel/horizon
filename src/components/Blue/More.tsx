...
export default function More(props: {
    ...
    const _handleCopyHorizonUrl = (e: SyntheticEvent) => {
        e.preventDefault();
        e.stopPropagation();

        const url = linkFromPost(post);
        navigator.clipboard.writeText(`https://vul.my.id/horizon/#${url}`);
    ...
    const _handlePin = async (e: SyntheticEvent) => {
        ...
        try {
            const result = await agent.upsertProfile(old => {
                ...
                return {
                    ...old,
                    horizonPinPost: isPin ? undefined : post.uri as string
                };
            })
                ...
                {dropdownOpen ?
                    <div className={styles.repostDropdown} onClick={e => e.stopPropagation()}>
                        {(post.author as any)?.did == user?.did ?
                            ...
                            : ''}
                        <p className="font-weight-bold" onClick={_handleCopyHorizonUrl}>
                            <img src={CopyIcon} alt="Copy" />
                            <span>Copy Horizon URL</span>
                        </p>
