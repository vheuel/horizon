...
export default function Blue(props: {
    ...
    const markdownText = () => {
        if ((post.record as any)?.horizonText) {
            const res = renderMarkdown((post?.record as any)?.horizonText);
            setMarkdown(res);
        } else {
    ...
    useEffect(() => {
        if (!(post.record as any)?.horizonAudio) return;
        let binary = convertURIToBinary((post.record as any)?.horizonAudio);
        let blob = new Blob([binary], {
            type: 'audio/ogg'
        });
        let blobUrl = URL.createObjectURL(blob);
        setAudio(blobUrl);
    }, [(post.record as any)?.horizonAudio]);
