    ...
    const _fetchImages = async () => {
        ...
        await Promise.all(
            record.data.map(async image => {
                if (!image.value.horizonType || image.value.horizonType != 'fleet') return;
                if (image.value.horizonImageUrl) {
                    newUrls.push({ url: image.value.horizonImageUrl, createdAt: image.value.createdAt });
                    return;
                }
                if (image.value.horizonImageString) {
                    newUrls.push({ url: image.value.horizonImageString, createdAt: image.value.createdAt });
                    return;
                }
